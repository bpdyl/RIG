...
from immnt_stkout_hist import immnt_stkout_hist_dm, immnt_stkout_hist_dwh
from overstk_hist import overstk_eoh_hist_dm, overstk_hist_dm, overstk_hist_dwh
from quintiles_hist import sls_txn_quintiles_hist
from rig_ddls import *
from stkout_hist import stkout_hist_dm, stkout_hist_dwh
from tqdm import tqdm
from utils import CustomFormatter, execute_query, log_file_path
...
rig_executor_handler = logging.FileHandler(f'{log_file_path}/rig_executor.log',
                                           encoding='utf-8')
...
def init_ins_query(init_queries, logger):
  ...
  for query in init_queries:
    ...
    # print(type(query))
    status, res = execute_query(cursor, query, log_prefix, attempt_date,
                                logger)
...
def ddls_and_insert_operation():
  ddls_and_ins_handler = logging.FileHandler(
      f'{log_file_path}/ddls_and_insert.log', encoding='utf-8')
  ...
  rig_ddls = dwh_ddls + dwh_v_ddls + dm_view_ddls + tmp_ddls + qntiles_tmp_ddl + qntiles_dwh_ddl + qntiles_view_ddl + qntiles_dm_view_ddl
...
def history_load():
  methods_group_1 = [(elgbl_mtx_load, (cursor, start_time))]

  methods_group_2 = [(avg_lw_sls_hist_dwh, (cursor, start_time))]

  methods_group_3 = [(immnt_stkout_hist_dwh, (cursor, start_time)),
                     (stkout_hist_dwh, (cursor, start_time)),
                     (overstk_hist_dwh, (cursor, start_time)),
                     (sls_txn_quintiles_hist, (cursor, start_time))]

  methods_group_4 = [(overstk_eoh_hist_dm, (cursor, start_time)),
  ...
  with concurrent.futures.ThreadPoolExecutor() as executor:
    ...
    futures_group_1 = {
        ...
        for method, args in methods_group_1
    }
    concurrent.futures.wait(futures_group_1)
    futures_group_2 = {
        executor.submit(method, *args)
        for method, args in methods_group_2
    }
    concurrent.futures.wait(futures_group_2)
    futures_group_3 = {
        executor.submit(method, *args)
        for method, args in methods_group_3
    }
    concurrent.futures.wait(futures_group_3)
    futures_group_4 = {
        executor.submit(method, *args)
        for method, args in methods_group_4
    }
...
def executor():
  ...
  tasks = ['ddls_and_insert_operation()', 'history_load()']
  # Set up the progress bar
  with tqdm(total=len(tasks), desc="Running tasks") as pbar:
    for task in tasks:
      # Execute the task
      eval(task)
